     ...
     2                  ztemp,zbeta,exact,xsurf,ypsurf,n,iend,ipos,tol)
      implicit none
      include 'const.dek'
...
c..locals variables
      ...
      parameter         (xdim=1000, ydim=2)

      double precision  xrk(xdim),yrk(ydim,xdim),bc(ydim),stptry,
     1                  stpmin,stpmax,odescal,lo,hi,zbrent,start,
...
c..the constant radial scale factor
      rscale = sqrt( f2*polyk * rhoc**(1.0d0/order - 1.0d0) )
