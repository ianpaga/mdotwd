...
!==============================================================================
  ...
  use Simulation_data, ONLY: sim_don_radius, sim_don_bdry, &
                             sim_acc_radius, sim_acc_bdry
...
!==============================================================================
  ...
  real    :: xx1,yy1,zz1,xx2,yy2,zz2,vx,vy,dr
...
!Turn off gravity if within donor or accretor, alleviates pileup in 4.3
!THIS ASSUMES YOU ARE USING A RADIUS CUT FOR THE BOUNDARY, NOT MASS
...
do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
    do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
       do i = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
   	  ...
   	  if (useMass1) then
   	      dr  = sqrt((xx(i)-xx1)**2 + (yy(j)-yy1)**2 + (zz(k)-zz1)**2)
	
	         if (dr .le. sim_acc_radius*sim_acc_bdry) then
	  ...
	  if (useMass2) then
	         dr  = sqrt((xx(i)-xx2)**2 + (yy(j)-yy2)**2 + (zz(k)-zz2)**2)
	
	         if (dr .le. sim_don_radius*sim_don_bdry) then
	           solnVec(accelIndex,i,j,k)  = 0.0
	         endif
